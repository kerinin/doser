...
func (waterLevelSensorL) LoadAutoTopOffs(ctx context.Context, e boil.ContextExecutor, singular bool, maybeWaterLevelSensor interface{}, mods queries.Applicator) error {
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.PumpID, &one.FillRate, &one.FillInterval, &one.MaxFillVolume, &one.Enabled, &one.Name, &one.FillLevelTimestamp, &one.FillLevelVolume, &localJoinCol)
		if err != nil {
